...
travelUp amount =
    travel { dir = Day3.Up, amount = amount } origin


travelRight amount =
    travel { dir = Day3.Right, amount = amount } origin
...
suite =
    describe "Day3"
        [ describe "part1"
            ...
            , test "test input 1" <|
                \_ -> Expect.equal (part1 testWire1Input testWire2Input) (Just 159)
            , test "test input 2" <|
                \_ -> Expect.equal (part1 testWire3Input testWire4Input) (Just 135)
            , test "actual input" <|
                \_ -> Expect.equal (part1 actualInput1 actualInput2) (Just 865)
            ]
        , describe "wire intersections"
            [ test "both go up" <|
                \_ ->
                    let
                        w1 =
